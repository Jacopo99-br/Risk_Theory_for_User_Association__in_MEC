...
class MEC:
    ...
    def C_(self,mul=1):
        ...
        initial_surplus=BUFFER_SIZE_MB/1000 #?? sarebbe B(0)
        n_CPU_CYCL=(User.MAX_TOTAL_DATA_SIZE_KB/1000)*(CYCLES_FOR_1_BIT*8*10**(7)) # num_bit approssimativi per ogni user * cicli per 1 bit

        '''premium_value=(n_CPU_CYCL*INTERVAL)/(CYCLES_FOR_1_BIT*8*10**(7))'''
        premium_value=1.33
